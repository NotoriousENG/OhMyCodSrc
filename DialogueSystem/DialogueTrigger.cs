...
public class DialogueTrigger : MonoBehaviour
{
    ...
    public Transform optionalIndicatorTransform;
    ...
    private void Start() 
    {
        if (optionalButtonIndicator != null)
        {
            ...
            indicator.transform.parent = optionalIndicatorTransform;
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        if (other.gameObject.tag == "Player" && Input.GetButton("Jump") && nextTime < Time.timeSinceLevelLoad)
        {
            if (!dialogueTiggered)
            {
                Debug.Log("Dialogue Triggered on stay");
                TriggerDialogue();
